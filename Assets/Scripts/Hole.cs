...
public class Hole : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        transform.position = new Vector3(StaticDefinitions.BEGIN_XPOS_OF_HOLE, 0.01f, 0);
    ...
    void FixedUpdate()
    {
        ...
        if (GameManager.Instance.openGateFlag && transform.position.x < StaticDefinitions.BEGIN_POSITION_OF_SECOND_GROUND)
        {
            ...
            if (holeWaitingWhileOpeningTheGate < Time.deltaTime)
            {
                if (timerForGoToSecondGround <= Time.deltaTime)
                {
                    ...
                    transform.position += new Vector3(StaticDefinitions.SPEED_OF_HOLE, 0, 0);
                    MainCamera.transform.position += new Vector3(StaticDefinitions.SPEED_OF_CAMERA, 0, 0);
    ...
    private bool checkBorder(bool _openGateFlag) 
    {
        ...
        if (!GameManager.Instance.openGateFlag)
        {
            ...
            if (transform.position.x < StaticDefinitions.FG_BACK_BORDER ) 
            {
                transform.position += new Vector3(0.04f, 0, 0); 
                return false;
            }
            else if (transform.position.x > StaticDefinitions.FG_TOP_BORDER) 
            {
                transform.position += new Vector3(-0.04f, 0, 0); 
                return false;
            }
            else if (transform.position.z < StaticDefinitions.FG_RIGHT_BORDER) 
            {
                transform.position += new Vector3(0, 0, 0.04f);
                return false;
            }
            else if (transform.position.z > StaticDefinitions.FG_LEFT_BORDER ) 
            {
                transform.position += new Vector3(0, 0, -0.04f);
                return false;
            }
        ...
        {
            if (transform.position.x < StaticDefinitions.SG_BACK_BORDER)
            {
                transform.position += new Vector3(0.04f, 0, 0);
                return false;
            }
            else if (transform.position.x > StaticDefinitions.SG_TOP_BORDER)
            {
                transform.position += new Vector3(-0.04f, 0, 0);
                return false;
            }
            else if (transform.position.z < StaticDefinitions.SG_RIGHT_BORDER)
            {
                transform.position += new Vector3(0, 0, 0.04f);
                return false;
            }
            else if (transform.position.z > StaticDefinitions.SG_LEFT_BORDER)
            {
                transform.position += new Vector3(0, 0, -0.04f);
                return false;
            }
